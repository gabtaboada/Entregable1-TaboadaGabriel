...
function obtenerCombos() {
    fetch(UrlCombos)
        ...
        .then(data => {
            ...
            /*
            if(mensajeCarrito =="armadoCombo"){
                console.log(" existe del mcarrito")
                let idPedido =1
                renderMenu(Menu, data)
                renderSeleccionCombo(idPedido,data)
            }
            else{
                //console.log("no existe del mcarrito")
...
function obtenerVerduras(idPedido, comboId, combosArmados) {
    //console.log(comboId)//aca tengo el id del combo seleccionado en el primer render
    fetch(UrlVerduras)
        .then(response => response.json())
        .then(data => {
...
function mostrarConfirmacion(idPedido, idCombo, combosArmados) {
    ...
    const comboNuevo = new comboPedido(idMax,idPedido, idCombo, busqueda.nombre,busqueda.descripcion, adicionales, busqueda.imagen,1,precioTotal,precioTotal)
...
function agregarCombo(comboNuevo) {
    ...
    */
    ...
    const carritoPrueba = []
    carritoPrueba.push(carritoRecuperado)
    console.log("CARRITO PRUEBA CON PUSH")
    console.log(carritoPrueba)
    carrito.push(comboNuevo)
    console.log("CARRITO")
    console.log(carrito)*/
    /* va */ 
